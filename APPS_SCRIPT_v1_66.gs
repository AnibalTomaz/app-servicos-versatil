...
function centralEnsureSheets_(ss){['META','ROOMS','CATEGORIES','PRODUCTS','CLOSURES','ORDERS','LEISURE'].forEach(n=>{if(!ss.getSheetByName(n))ss.insertSheet(n)})}
...
function normalizeLeisureV166_(rows){
  const fallbackPhone='(65) 3313-0050 (Sec. de Turismo)';
  return (Array.isArray(rows)?rows:[]).map(n=>({
    ...n,
    name:String(n.name||'').replace(/Laser e Conveni[eê]ncia/gi,'Lazer e Conveniências'),
    phone:(n.type==='place'&&!String(n.phone||'').trim())?fallbackPhone:n.phone,
    sort:Number(n.sort||0)
  }));
}

function centralLeisureGuideV165_(){
  const done=centralMetaGet('LEISURE_COMPLETE_SEED_V165');
  if(String(done)!=='1'){
    const seed=DEFAULT_LEISURE_GUIDE_V165;
    ...
    centralMetaSet('LEISURE_COMPLETE_SEED_V165','1');
    return normalizeLeisureV166_(seed);
  }
  const rows=centralReadObjects_('LEISURE');
  return rows.length?normalizeLeisureV166_(rows):normalizeLeisureV166_(DEFAULT_LEISURE_GUIDE_V165);
}
function centralReadPublicData(){
  return {rooms:centralReadObjects_('ROOMS'),categories:centralReadObjects_('CATEGORIES'),products:centralReadObjects_('PRODUCTS').map(p=>({...p,q:Number(p.q||0),a:Number(p.a||0),halfDay:p.halfDay===true||String(p.halfDay).toLowerCase()==='true'})),availabilityClosures:centralReadObjects_('CLOSURES'),leisureGuide:centralLeisureGuideV165_()};
}
