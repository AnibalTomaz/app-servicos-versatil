...
function enviarPostalV170_(data){
  ...
  const html=emailShell('Um postal de Cuiabá','Correio Versátil',
    `<p>Olá, ${escaparHtml(recipientName||'amigo(a)')}.</p><p>${escaparHtml(msg)}</p><p><b>Abraços, ${escaparHtml(senderName)}</b></p><hr><p>${escaparHtml(cfg.address||'')}</p><p>Reservas: ${escaparHtml(cfg.phone||'')}</p><p><b>${escaparHtml(cfg.couponText||'Cupom')}: ${escaparHtml(coupon)}</b></p><p><a href="https://www.starlis.com.br">www.starlis.com.br</a></p>`);
  const plain=`Olá, ${recipientName||'amigo(a)'}.\n\n${msg}\n\nAbraços, ${senderName}\n\n${cfg.address||''}\nReservas: ${cfg.phone||''}\n${cfg.couponText||'Cupom'}: ${coupon}\nhttps://www.starlis.com.br`;
  enviarHtml(recipient,'Um postal de Cuiabá - Correio Versátil',plain,html);
  const admin=ADMIN_EMAILS.filter(Boolean)[0];
  if(admin)enviarHtml(admin,'Cópia - Correio Versátil',`Postal enviado por ${senderName} (${senderEmail}) para ${recipientName} (${recipient}). Cupom: ${coupon}`,html);
}
...
function centralLeisureGuideV167_(){
  ...
  return rows.length?rows.map(n=>({...n,sort:Number(n.sort||0)})):DEFAULT_LEISURE_GUIDE_V167;
}
