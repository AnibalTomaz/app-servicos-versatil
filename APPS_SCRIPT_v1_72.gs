...
function enviarPostalV170_(data){
  ...
  const front=`<div style="padding:16px;background:#e7d5ad;border:1px solid #b9ad99;border-radius:8px;text-align:center"><div style="font-size:22px;font-weight:bold">Um presente para você!</div><div style="margin-top:12px;background:#fff;padding:22px;border-radius:6px"><b>Starlis Home Versátil</b><br>Reserve já.</div></div>`;
...
function centralLeisureGuideV167_(){
  ...
  const missing=EXTRA_LEISURE_GUIDE_V171.filter(n=>!ids.has(n.id));
  ...
  return rows.length?rows.map(n=>({...n,sort:Number(n.sort||0)})):DEFAULT_LEISURE_GUIDE_V167.concat(EXTRA_LEISURE_GUIDE_V171);
}
