...
function doPost(e){
  try{
    ...
    else if(data.action==='setAvailability'){sincronizarDisponibilidade(data)}
...
function enviarEmailsPedido(data){
  ...
  if(c.email)MailApp.sendEmail({to:c.email,subject:'Solicitação recebida - Serviços Versátil',body:corpoTextoPedido(data,true),htmlBody:clienteHtml,name:APP_NAME});
  [...new Set(ADMIN_EMAILS.filter(Boolean))].forEach(email=>MailApp.sendEmail({to:email,subject:'Novo pedido - Serviços Versátil',body:corpoTextoPedido(data,false),htmlBody:adminHtml,name:APP_NAME}));
}
...
function sincronizarDisponibilidade(data){
  const operation=data.operation||'close',closures=data.closures||[],props=PropertiesService.getScriptProperties(),cal=CalendarApp.getDefaultCalendar();
  closures.forEach(c=>{
    const key='CLOSURE_EVENT_'+c.id;
    if(operation==='close'){
      if(props.getProperty(key))return;
      const ev=cal.createEvent(`INDISPONÍVEL - ${c.scopeLabel||'Itens'} - ${c.periodLabel||''}`,dataHora(c.date,c.calendarStart||'06:00'),dataHora(c.date,c.calendarEnd||'12:00'),{description:`${APP_NAME}\nBloqueio de disponibilidade\nData: ${formatarData(c.date)}\nPeríodo: ${c.periodLabel||''}\nEscopo: ${c.scopeLabel||''}\nTipo: ${c.scopeType||''}`});
      props.setProperty(key,ev.getId());
    }else{
      const id=props.getProperty(key);if(!id)return;
      try{const ev=cal.getEventById(id);if(ev)ev.deleteEvent()}catch(e){console.error(e)}
      props.deleteProperty(key);
    }
  });
}
