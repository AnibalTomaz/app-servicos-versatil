...
const CURRENT_APP_VERSION='2.82';

const VERSION_PUBLICATION_PROP='VERSATIL_VERSION_PUBLICATION_V275';
const VERSION_PUBLICATION_TZ='America/Sao_Paulo';

function versionPublicationStampV275_(){
  const props=PropertiesService.getScriptProperties();
  let raw=props.getProperty(VERSION_PUBLICATION_PROP)||'';
  let obj={};
  try{obj=raw?JSON.parse(raw):{}}catch(e){obj={}}
  if(String(obj.version||'')!==String(CURRENT_APP_VERSION)||!obj.publishedAt){
    obj={
      version:String(CURRENT_APP_VERSION),
      publishedAt:Utilities.formatDate(new Date(),VERSION_PUBLICATION_TZ,"yyyy-MM-dd'T'HH:mm:ssXXX")
    };
    props.setProperty(VERSION_PUBLICATION_PROP,JSON.stringify(obj));
  }
  ...
  try{
    ...
    if(!action){
      const payload={
        ...
        publicationAt:versionPublicationStampV275_(),
...
function salvarRoteiro_(data){
  ...
  if(sh.getLastRow()===0){
    sh.appendRow(['id','createdAt','clientName','roomName','itemCount','itemsJson','excludeFromStatistics','clientEmail']);
  }else{
    if(sh.getLastColumn()<7)sh.getRange(1,7).setValue('excludeFromStatistics');
    if(sh.getLastColumn()<8)sh.getRange(1,8).setValue('clientEmail');
  }
  ...
  sh.appendRow([
    ...
    String(data.client?.roomName||''),
    items.length,
    JSON.stringify(items.map(x=>({id:x.id||'',name:x.name||''}))),
    data.excludeFromStatistics===true,
    String(data.client?.email||'').trim()
  ]);
}
...
function enviarEmailRoteiro_(data){
  ...
  const clientName=String(data.client?.name||'Hóspede').trim();
  const roomName=String(data.client?.roomName||'').trim();
  const items=Array.isArray(data.items)?data.items:[];
  ...
  const subject='Meu Roteiro - Serviços Versátil';
  const text='Seu roteiro em Cuiabá:\\n\\n'+items.map((x,i)=>`${i+1}. ${x.name}`).join('\\n');
  const html=emailShell('Seu roteiro em Cuiabá','Aproveite sua estada!',`<p>Olá, ${escaparHtml(clientName)}.</p><p>Segue o roteiro que você criou no APP SERVIÇOS VERSÁTIL.</p><table style="width:100%;border-collapse:collapse">${rows}</table><p style="margin-top:18px"><b>Aproveite sua estada em Cuiabá!</b></p>`);
  enviarHtml(email,subject,text,html);

  const adminText=`Roteiro criado no APP SERVIÇOS VERSÁTIL.\\nCliente: ${clientName}\\nUnidade: ${roomName}\\nE-mail: ${email}\\n\\n${items.map((x,i)=>`${i+1}. ${x.name}`).join('\\n')}`;
  const adminHtml=emailShell(
    'Cópia de roteiro criado',
    `${escaparHtml(clientName)}${roomName?' • '+escaparHtml(roomName):''}`,
    `<p><b>Cliente:</b> ${escaparHtml(clientName)}</p>
     <p><b>Unidade:</b> ${escaparHtml(roomName)}</p>
  ...
  ADMIN_EMAILS
    ...
    .filter(addr=>addr.toLowerCase()!==email.toLowerCase())
    .forEach(addr=>enviarHtml(addr,`Cópia - ${subject}`,adminText,adminHtml,email));
}
...
function itinerariesTodayV170_(){
  ...
  return rows.map(r=>{const d=new Date(r[1]);if(isNaN(d)||Utilities.formatDate(d,APP_TIMEZONE,'yyyy-MM-dd')!==today)return null;
    return {time:Utilities.formatDate(d,APP_TIMEZONE,'HH:mm'),clientName:String(r[2]||''),clientEmail:String(r[7]||''),roomName:String(r[3]||''),itemCount:Number(r[4]||0)};}).filter(Boolean).reverse();
}
