...
const CURRENT_APP_VERSION='1.96';
...
function doPost(e){
  try{
    ...
    else if(data.action==='sendPostcard'){enviarPostalV170_(data);return jsonResponse({ok:true,action:'sendPostcard'})}
...
function enviarPostalV170_(data){
  ...
  const senderName=String(data.sender?.name||'Hóspede'),senderEmail=String(data.sender?.email||'').trim(),coupon=String(data.coupon||'');
  ...
  const cfg=data.config||{};if(!String(cfg.message||'').trim())cfg.message='Caro amigo(a) estou em Cuiabá, ficando no Versátil e estou adorando o lugar. Envio este postal pois lembrei de você durante minha viagem. Espero que estejas bem, mande notícias!';if(!String(cfg.couponText||'').trim())cfg.couponText='Use meu cupom de desconto e ganhe um up-grade na sua reserva';const subject=`Você recebeu um postal de ${senderName}`,site='https://www.starlis.com.br';
  const thank=`mailto:${senderEmail}?subject=${encodeURIComponent('Obrigado pelo postal!')}`;
  const park="https://hotelmt.com.br/wp-content/uploads/2020/03/original-ff169b7274cfb6259fe8c25c3c29ac1e.jpeg",facade="https://cf.bstatic.com/xdata/images/hotel/max1024x768/527400490.jpg?k=601e3832271bf87f77452863c49ee6daf8c1cd63020105c24ada5f36f35a6777&o=",pool="https://images.trvl-media.com/lodging/38000000/37200000/37198900/37198826/ff5edf34_edited_0d15.jpg?impolicy=resizecrop&ra=fit&rw=598";
  const front=`<div style="padding:8px;background:#e7d5ad;border:1px solid #b9ad99;border-radius:8px">
    ...
    <table role="presentation" width="100%" cellspacing="6" cellpadding="0"><tr>
      <td width="65%" rowspan="2" style="vertical-align:top"><img src="${park}" alt="Parque das Águas" style="width:100%;max-height:420px;object-fit:cover;border-radius:5px"></td>
      <td width="35%" style="vertical-align:top"><div style="position:relative"><img src="${facade}" alt="Starlis Home Versátil" style="width:100%;max-height:260px;object-fit:contain;background:#eee;border-radius:5px"><div style="color:#444;padding:4px 0 0;text-align:center;font-family:cursive"><b>Reserve já.</b> • Starlis Home Versátil • www.starlis.com.br</div></div></td>
    </tr><tr><td><img src="${pool}" alt="Piscina Starlis Home Versátil" style="width:100%;max-height:150px;object-fit:cover;border-radius:5px"></td></tr></table>
  </div>`;
  const back=`<div style="padding:18px;background:#fffaf1;border:1px solid #b9ad99;border-radius:8px;font-family:cursive;color:#174f9a"><p>Olá, ${escaparHtml(recipientName||'amigo(a)')}.</p><p>${escaparHtml(String(cfg.message||''))}</p><p><b>Abraços, ${escaparHtml(senderName)}</b></p><p>${escaparHtml(cfg.address||'')}<br>Reservas: ${escaparHtml(cfg.phone||'')}</p><div style="border:1px dashed #174f9a;padding:10px"><b>${escaparHtml(cfg.couponText||'Cupom')}: ${escaparHtml(coupon)}</b><br><br><a href="${site}" style="display:inline-block;background:#10578f;color:#fff;text-decoration:none;padding:10px 16px;border-radius:7px">Quero utilizar!</a></div></div>`;
  ...
  enviarHtml(recipient,subject,`Um presente para você!\n\n${cfg.message||''}\n\nAbraços, ${senderName}\nCupom: ${coupon}\n${site}`,html);
  const admin=ADMIN_EMAILS.filter(Boolean)[0];if(admin)enviarHtml(admin,`Cópia - ${subject}`,`Postal enviado por ${senderName} (${senderEmail}) para ${recipientName} (${recipient}). Cupom: ${coupon}`,html);
}
...
function centralEnsureSheets_(ss){['META','ROOMS','CATEGORIES','PRODUCTS','CLOSURES','ORDERS','LEISURE','ITINERARIES','CONTENT'].forEach(n=>{if(!ss.getSheetByName(n))ss.insertSheet(n)})}
...
function centralSavePublicData(d){
  ...
  centralWriteRows_('CONTENT',['key','json'],content);
...
function centralLeisureGuideV167_(){
  ...
  const missing=EXTRA_LEISURE_GUIDE_V171.concat(EXTRA_LEISURE_GUIDE_V173,EXTRA_LEISURE_GUIDE_V196).filter(n=>!ids.has(n.id));
