...
function renderPackageUseRow(p,index,date=today()){
  ...
  return `<div class="package-use-row" data-package-use="${index}">
    <div class="package-use-title">Utilização ${index+1}</div><button class="cat leisure-entry-card" onclick="page='leisure';render()"><div class="ico">📍</div><h3>Lazer e Conveniênciass</h3><p>Turismo, alimentação, conveniências e espaços de eventos</p></button>
    <div class="field">
      <label>Data</label>
...
function leisurePlaceHtml(p){
  const fallback=leisureFallbackImage(p);
  const photo=`<img class="leisure-place-img" src="${esc(p.imageUrl||fallback)}" alt="${esc(p.name)}" data-fallback="${esc(fallback)}" onerror="if(this.src.indexOf(this.dataset.fallback)<0){this.src=this.dataset.fallback}else{this.onerror=null}">`;
  return `<details class="leisure-place" data-leisure-id="${esc(p.id)}"><summary>${esc(p.name)}<span>›</span></summary>
    <div class="leisure-place-body"><div class="leisure-place-grid">
      ...
      <div>
        ...
        <div class="leisure-meta"><b>Distância do Versátil:</b> ${esc(p.distance||'Consulte no Google Maps')}<br><b>Telefone:</b> ${esc(p.phone||'(65) 3313-0050 (Sec. de Turismo)')}</div>
...
function leisureNodeHtml(node,depth=1){
  ...
  return `<details class="leisure-level-${Math.min(depth,5)}" data-leisure-id="${esc(node.id)}"><summary>${esc(node.name)}<span>⌄</span></summary>
...
function leisureGuidePage(){
  ...
  return `<div class="card leisure-guide-card">
    <div class="row between leisure-guide-head"><div><h2>Lazer e Conveniênciass</h2><div class="muted">Guia de consulta do hóspede</div></div>
    <button class="btn" onclick="page='catalog';render()">Voltar ao catálogo</button></div>
    <div class="notice leisure-notice"><b>Encontre tudo ao seu redor.</b></div>
...
function leisureAdmin(){
  ...
  return `<div class="card"><div class="row between">
    <div><h2 style="margin:0">Lazer e Conveniênciass</h2><p class="muted">${total} locais finais cadastrados. Edite menus, especialidades e estabelecimentos.</p></div>
...
function adminView(){let menus=[
  ...
  ['leisure','Lazer e Conveniênciass'],
  ['calendar','Calendário'],
  ['orders','Pedidos'],
...
function bootVersatilV140(){
  ...
  try{
    ...
    console.info('APP SERVIÇOS VERSÁTIL - Versão 1.66 carregada.');
  }catch(err){
