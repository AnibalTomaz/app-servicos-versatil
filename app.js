...
function leisurePlaceHtml(p){
  const photo=p.imageUrl
    ?`<img class="leisure-place-img" src="${esc(p.imageUrl)}" alt="${esc(p.name)}" onerror="this.outerHTML='<div class=&quot;leisure-photo-missing&quot;>Foto indisponível</div>'">`
    :`<div class="leisure-photo-missing">Foto a cadastrar</div>`;
  return `<details class="leisure-place"><summary>${esc(p.name)}<span>›</span></summary>
    <div class="leisure-place-body"><div class="leisure-place-grid">
      ...
      <div>
        ...
        <div class="leisure-meta"><b>Distância do Versátil:</b> ${esc(p.distance||'A verificar')}<br><b>Telefone:</b> ${esc(p.phone||'A verificar')}</div>
...
function leisureNodeHtml(node,depth=1){
  ...
  return `<details class="leisure-level-${Math.min(depth,5)}"><summary>${esc(node.name)}<span>⌄</span></summary>
...
function leisureGuidePage(){
  ...
  return `<div class="card leisure-guide-card">
    ...
    <div class="notice leisure-notice"><b>Explore Cuiabá e região.</b><br>Abra as categorias até chegar ao local desejado. Esta área é somente para consulta.</div>
...
function bootVersatilV140(){
  ...
  try{
    ...
    console.info('APP SERVIÇOS VERSÁTIL - Versão 1.65 carregada.');
  }catch(err){
