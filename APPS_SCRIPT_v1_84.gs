...
const CURRENT_APP_VERSION='1.84';
...
function centralLeisureGuideV167_(){
  ...
  const missing=EXTRA_LEISURE_GUIDE_V171.concat(EXTRA_LEISURE_GUIDE_V173).filter(n=>!ids.has(n.id));
  ...
  return rows.length?rows.map(n=>({...n,sort:Number(n.sort||0)})):DEFAULT_LEISURE_GUIDE_V167.concat(EXTRA_LEISURE_GUIDE_V171,EXTRA_LEISURE_GUIDE_V173);
}
